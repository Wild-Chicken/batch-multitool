...
rem 	nothing
rem Removed:
rem 	removed temp delete on test and pid kill
rem Improved/Changed:
rem 	made deletion time for pictures 2s>5s
rem Bug Fixes
rem 	redid the ENTIRE pid kill, because batch doesnt support nesting set /p in set /p
rem 	removed a line that deleted a folder that might break uninstalling apps or reinstalling them
rem 	fixed a secrect
rem Notes
...
if "%wow%"=="0" (
	...
	echo  once you are done killing the pid or app name, you can close me!
    start %temp%\pid.bat
) else if "%wow%"=="5" (
	echo great choice!
