@echo off
:: v3.23.1 Minor Convienience
:: Secrect Count: 15
:: Added:
::	*** for recommened admin
::	wipe all script data
::	tiny things
:: Removed:
::	(you agreed to this) in analytics
::	spaces near the ascii art version number (how else should i name it????)
::	tiny things
:: Improved/Changed:
::	rephrashed no admin #3
::	clean the files.. to clean temp files
::	rearranged messages near ascii art
::	removed ( and ) for single line commands, now its harder to skid my script, slightly.
::	combined script never ran and script first run time
::	shorter uac restart cmd
::	tiny things
:: Bug Fixes
::	nothing
:: Notes
...
set /p rs=<!sp!\rs.txt
if %errorlevel%==0 ( echo script ran before
...
echo 	  __            __			v%ver% Minor Convienience
echo 	 / /____  ___  / /  ^> coded by chicken
echo 	/ __/ _ \/ _ \/ /  ^> hope you enjoy!
echo 	\__/\___/\___/_/
echo 	       ____   ___  ____  ______
echo 	 _  __^|_  /  ^|_  ^|^|_  / ^<  / _ \
echo 	^| ^|/ //_ ^<_ / __/_/_ ^<_ / / , _/ ^> 17 for my socials
echo 	^|___/____(_)____/____(_)_/_/^|_^| ^> 27 to send me a message
echo.
