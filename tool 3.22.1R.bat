@echo off
:: Minor Improvments Update v3.22.1
:: Secrect Count: 12 > 13
:: Added:
:: 	stuff to ascii art at the start
::	extra space at ascii art art
::	tos/pop/name calling/stats
::	secrect
:: Removed:
:: 	minor things
:: Improved/Changed:
...
::	pscmd to pc
::	del to dl
::	cleaned up the download system a bit
::	abbreviated stuff
::	did some other little things
:: Bug Fixes
:: 	yes! i did it, because of setlocal i can improve the secrect system a bit more!
::	fixed secrects not working in 3.22 (my bad)
::	fixed touchscreen not working in 3.22 (again, my bad)
:: Notes
...
title tool 3.22.1R ^| by chicken
:: if you want to know what something does, dm me using my socials in #17!
if not exist %programdata%\script (
	md %programdata%\script
)
if exist %programdata%\script\pptos.txt (
	echo PP and TOS already accepted.
) else ( goto pptos
)
if not exist %programdata%\script\name.txt (
...
echo      __              __
echo     / /_____  ____  / /
echo    / __/ __ \/ __ \/ / ^> coded by
echo   / /_/ /_/ / /_/ / / ^> chicken
echo   \__/\____/\____/_/    _______
echo     ^|__  / ^|__ \^|__ \  ^<  / __ \
echo      /_ ^<  __/ /__/ /  / / /_/ /
echo    ___/ / / __// __/_ / / _, _/
echo   /____(_)____/____(_)_/_/ ^|_^| ^> enjoy
if defined name (
	echo.
	echo hello, !name!
) else ( echo.
